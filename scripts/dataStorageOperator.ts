...
import { DataStorageOperator as DataStorageOperatorContract} from "../typechain";
...
export class DataStorageOperator {
  ...
  async getFeeConfig() {
    const dataStorageContract = <DataStorageOperatorContract>await ethers.getContract("DataStorageOperator")
    return dataStorageContract.feeConfig()
  }

  async getTimepoint(index: BigNumberish) {
    return this.timepoints[Number(index)]
  }
  ...
  async write(
    ...
    const _last = await this.getTimepoint(index);
    ...
    const avgTick = await this._getAverageTick(blockTimestamp, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative);
    let prevTick = tick;
    if (index != oldestIndex) {
      ...
      prevTick = (last.tickCumulative - _prevLastTickCumulative) / (last.blockTimestamp - _prevLastBlockTimestamp);
    }

    this.timepoints[Number(indexUpdated)] = DataStorageOperator.createNewTimepoint(last, blockTimestamp, tick, prevTick, liquidity, avgTick, volumePerLiquidity);
  ...
  async _getSingleTimepoint(
    ...
    if (secondsAgo == 0n || DataStorageOperator.lteConsideringOverflow((await this.getTimepoint(index)).blockTimestamp, target, time)) {
      const last = await this.getTimepoint(index);
      if (last.blockTimestamp == target) {
        return last;
      } else {
        // otherwise, we need to add new timepoint
        const avgTick = await this._getAverageTick(time, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative);
        let prevTick = tick;
        {
          if (index != oldestIndex) {
            const _prevLast = await this.getTimepoint(index - 1n); // considering index underflow
            prevTick = (last.tickCumulative - _prevLast.tickCumulative) / (last.blockTimestamp - _prevLast.blockTimestamp);
          }
        }
    ...
    assert(DataStorageOperator.lteConsideringOverflow((await this.getTimepoint(oldestIndex)).blockTimestamp, target, time), 'OLD');
    ...
    if (target != beforeOrAt.blockTimestamp) {
      ...
      beforeOrAt.secondsPerLiquidityCumulative += (
        ((atOrAfter.secondsPerLiquidityCumulative - beforeOrAt.secondsPerLiquidityCumulative) * targetDelta) / timepointTimeDelta
      );
  ...
  static createNewTimepoint(
    ...
    last.tickCumulative += tick * delta;
    last.secondsPerLiquidityCumulative += ((delta << 128n) / (liquidity > 0n ? liquidity : 1n)); // just timedelta if liquidity == 0
    last.volatilityCumulative += DataStorageOperator._volatilityOnRange(delta, prevTick, tick, last.averageTick, averageTick); // always fits 88 bits
