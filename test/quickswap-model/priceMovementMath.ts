...
export class PriceMovementMath {
	...
	static getNewPriceAfterInput(price: bigint, liquidity: bigint, input: bigint, zeroToOne: boolean) {
	...
	static getNewPriceAfterOutput(price: bigint, liquidity: bigint, output: bigint, zeroToOne: boolean) {
		return this.getNewPrice(price, liquidity, output, zeroToOne, false);
	}

	static getNewPrice(price: bigint, liquidity: bigint, amount: bigint, zeroToOne: boolean, fromInput: boolean) {
		assert(price > 0);
		assert(liquidity > 0);
			...
			if (fromInput) {
				...
				if ((product = amount * price) / amount == price) {
					...
					if (denominator >= liquidityShifted)
						return FullMath.mulDivRoundingUp(liquidityShifted, price, denominator); // always fits in 160 bits
				}
	...
	static getTokenADelta01(to: bigint, from: bigint, liquidity: bigint) {
		return TokenDeltaMath.getToken0DeltaRoundUp(to, from, liquidity, true);
	}

	static getTokenADelta10(to: bigint, from: bigint, liquidity: bigint) {
		return TokenDeltaMath.getToken1DeltaRoundUp(from, to, liquidity, true);
	}

	static getTokenBDelta01(to: bigint, from: bigint, liquidity: bigint) {
		return TokenDeltaMath.getToken1DeltaRoundUp(to, from, liquidity, false);
	}

	static getTokenBDelta10(to: bigint, from: bigint, liquidity: bigint) {
