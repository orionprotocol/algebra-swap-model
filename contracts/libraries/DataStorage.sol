...
library DataStorage {
	...
	function getSingleTimepoint(
		...
		if (secondsAgo == 0 || lteConsideringOverflow(self[index].blockTimestamp, target, time)) {
			...
			if (last.blockTimestamp == target) {
				...
				int24 avgTick = int24(
					_getAverageTick(self, time, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative)
				);
				int24 prevTick = tick;
				{
	...
	function write(
		...
		int24 avgTick = int24(
			_getAverageTick(self, blockTimestamp, tick, index, oldestIndex, last.blockTimestamp, last.tickCumulative)
		);
		int24 prevTick = tick;
		if (index != oldestIndex) {
			Timepoint storage _prevLast = self[index - 1]; // considering index underflow
