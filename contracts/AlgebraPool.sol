...
contract AlgebraPool is PoolState, PoolImmutables, IAlgebraPool {
	...
	function _recalculatePosition(
		...
		(uint128 currentLiquidity, uint32 lastLiquidityAddTimestamp) = (
			_position.liquidity,
			_position.lastLiquidityAddTimestamp
		);
		...
		if (innerFeeGrowth0Token != _innerFeeGrowth0Token) {
			...
			fees0 = uint128(
				FullMath.mulDiv(innerFeeGrowth0Token - _innerFeeGrowth0Token, currentLiquidity, Constants.Q128)
			);
		}
		uint128 fees1;
		if (innerFeeGrowth1Token != _innerFeeGrowth1Token) {
			...
			fees1 = uint128(
				FullMath.mulDiv(innerFeeGrowth1Token - _innerFeeGrowth1Token, currentLiquidity, Constants.Q128)
			);
		}
		...
		returns (uint256 amount0, uint256 amount1, uint128 liquidityActual)
	{
...

		{
			(, int256 amount0Int, int256 amount1Int) = _updatePositionTicksAndFees(
				...
				bottomTick,
				topTick,
				int256(liquidityActual).toInt128()
			);
	...
	function burn(
		...
		if (amount0 | amount1 != 0) {
			(position.fees0, position.fees1) = (
				position.fees0.add128(uint128(amount0)),
				position.fees1.add128(uint128(amount1))
			);
		}
	...
	function _writeTimepoint(
		...
		return
			IDataStorageOperator(dataStorageOperator).write(
				timepointIndex,
				blockTimestamp,
				tick,
				liquidity,
	...
	{
		return
			IDataStorageOperator(dataStorageOperator).getSingleTimepoint(
				blockTimestamp,
				secondsAgo,
				startTick,
				timepointIndex,
	...
	function swap(
		...
		(amount0, amount1, currentPrice, currentTick, currentLiquidity, communityFee) = _calculateSwapAndLock(
			zeroToOne,
			amountRequired,
			limitSqrtPrice
		);
